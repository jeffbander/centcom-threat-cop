...
export function computePriorityScore(input: PriorityInputs): number {
  ...
  const recency = 40 * Math.pow(0.5, ageMs / RECENCY_HALF_LIFE_MS);
  const sources = Math.min(25, Math.max(0, input.sourceCount) * 5);
  const regional = Math.min(15, Math.max(0, input.regionalActivity ?? 0) * 3);
  const raw =
...
export function explainPriority(input: PriorityInputs): string {
  ...
  return `Ranked highly because ${parts.join(", ")}. This score does not predict future events.`;
}
